"""
Optuna hyperparameter sweep for NucleaseConvNet with cluster-safe checkpointing.

The train/val split is identical to the transformer sweep: mutation_count_split
with max_train_mutations=2, val_frac=0.1, seed=42. This lets you compare the
best CNN and best transformer on the same held-out set without any data leakage.

Each trial saves a checkpoint after every epoch to checkpoints/sweep_cnn/current_trial.pt.
...
Results are written to checkpoints/sweep_cnn/results.csv and to an Optuna SQLite DB.
Multiple Slurm jobs can attach to the same study by pointing at the same DB file.

Usage:
    python scripts/run_sweep_cnn.py              # create/join study + run 20 trials
    python scripts/run_sweep_cnn.py --count 50
    python scripts/run_sweep_cnn.py --study_name my_cnn_study
"""
...
from nucb_cnn.training.metrics import ACTIVITY_CLASSES, hit_rate, spearman_rho
from nucb_cnn.training.trainer import _warmup_cosine_lr, load_config

optuna.logging.set_verbosity(optuna.logging.WARNING)

_CKPT_DIR     = "checkpoints/sweep_cnn"
...
_CSV_FIELDS = [
    ...
    "learning_rate", "weight_decay", "num_filters", "kernel_size",
    "num_conv_layers", "hidden_dim", "dropout",
]
...
def objective(trial: optuna.Trial) -> float:
    ...
    num_conv_layers = trial.suggest_categorical("num_conv_layers", [2, 3, 4, 6])
    ...
    cfg["model"]["num_conv_layers"] = num_conv_layers
    ...
    print(
        f"\n[Trial {trial.number}] "
        f"lr={lr:.2e}  wd={wd:.2e}  filters={num_filters}  "
        f"kernel={kernel_size}  layers={num_conv_layers}  "
        f"hidden={hidden_dim}  dropout={dropout:.3f}",
        flush=True,
    )
    ...
    # Identical split to the transformer sweep — same function, same seed, same params.
    df = load_landscape()
    ...
    model = NucleaseConvNet(**cfg["model"]).to(device)
