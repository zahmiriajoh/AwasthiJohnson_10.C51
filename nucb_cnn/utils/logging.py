...
def setup_logger(name: str = "nucb_cnn", level: int = logging.INFO) -> logging.Logger:
...
def setup_wandb(cfg: dict) -> Any | None:
    ...
    try:
        import wandb
        return wandb.init(
            project=cfg["training"].get("wandb_project", "nuclease_cnn"),
            config=cfg,
        )
    except ImportError:
        logging.getLogger("nucb_cnn").warning(
